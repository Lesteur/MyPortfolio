...
SDL_Surface * CreerImage(char * str) {
    SDL_Surface * imgSurface = IMG_Load(str);
    if (imgSurface == NULL) {
        fprintf(stderr, "Erreur lors de la création de la surface d'image : %s\n", TTF_GetError());
    }
    return imgSurface;
}
...
// Initialise les différentes librairies SDL
void SDL_InitAll() {
