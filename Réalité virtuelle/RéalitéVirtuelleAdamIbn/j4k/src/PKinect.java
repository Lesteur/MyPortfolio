...
public class PKinect extends J4KSDK{
	...
	public PSkeleton[] getPSkeletons()
	{
		...
		if(s==null) {PSkeleton[] ps=new PSkeleton[this.getSkeletonCountLimit()];for(int i=0;i<this.getSkeletonCountLimit();i++)ps[i]=new PSkeleton();return ps;}
	...
	public PDepthMap getPDepthMap()
	{
		...
		if(xyz==null) return new PDepthMap(getDepthWidth(),getDepthHeight());
		float[] uv=this.getUV();
		PDepthMap map=new PDepthMap(getDepthWidth(), getDepthHeight(),xyz);
	...
	public void updatePImage(PImage pimg,byte[] pixels)
	{
		...
		if(video_frame==null || video_frame.getWidth()!=getColorWidth())
		{
			video_frame=new BufferedImage(getColorWidth(),getColorHeight(),BufferedImage.TYPE_4BYTE_ABGR);
	...
	{
		byte[] pixels=this.getColorFrame();
